...
class DBSCAN(ClusterMixin, BaseEstimator):
    ...
    def build_clusters(self):
        ...
        for pos in set(self.labels_):
            if pos != -1:
                ...
                wordhas = self.allwords[pos_list[0]]["word"] + str(
                ...
                self.es.index(
                    index=self.cluster_name_index, id=cluster["id"], body=cluster
                )
            else:
        ...
        if len(n_neighbors) == 0:
            ...
            if on_include == False:
                ...
                for neighbor in n_neighbors:
                    ...
                    self.es.delete(
                        index=self.cluster_name_index, id=neighbor["cluster_id"]
                    )
    ...
    def epsilonVoisinage(self, P):
        ...
        for e in self.allwords:
            ...
            if (
                distance.cosine(
                    self.vectors.query(e["word"]), self.vectors.query(P["word"])
                )
                < self.eps
            ):
                voisins.append(e)
